...
function EditarGrupoFuncionario(type: any) {
  ...
  const [user, setUser] = useState<{ id: any; name: any; }[]>([]);
  const [ids, setIds] = useState<{ id: any; name: any; }[]>([]);
    ...
    const fetchData = async () => {
      try {
        const [response1, response2] = await Promise.all([
          axios.get(`${type.urlFun}${id}`),
          axios.get(type.urlUser),
        ]);
  
        const mapData = (data: any) => ({ id: data.id, name: data.user.userEmail });
  
        const ids = response1.data.map(mapData);
        setIds((prevIds) => [...prevIds, ...ids]);
        setUser((prevUser) => [...prevUser, ...ids]);
        setUserOptions((prevOptions) => [...prevOptions, ...response1.data.map((item: any) => item.user.userEmail)]);
  
        console.log(response1.data);
        const { groupName, groupType, groupDescription } = response1.data[0].group;
        setGroupName(groupName);
        setGroupType(groupType);
        setGroupDescription(groupDescription);
  
        const users = response2.data.map((item: any) => ({
          id: item.id,
